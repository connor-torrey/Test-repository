...
#ifndef INCLUDE__VT3_POU__SLAVECONFIGFB_CPU0_C
#define INCLUDE__VT3_POU__SLAVECONFIGFB_CPU0_C
...
static const vt3_screen_initializer FAR __init_table__slaveConfigFB_CPU0[] = {
	{ NULL, NULL, 0 } /* sentinel */
}; /* __init_table__slaveConfigFB_CPU0[] */
...
static const slaveConfigFB_CPU0_t FAR __const__slaveConfigFB_CPU0__param_init = 
{
	/* EN                   */ TRUE,
	/* ENO                  */ TRUE,
	/* nodeid               */ ((USINT)0U),
}; /* end of __const__slaveConfigFB_CPU0__param_init */
...
void slaveConfigFB_CPU0__init(void)
{
	...
	for ( p = __init_table__slaveConfigFB_CPU0; p->len; p ++ )
	{
		FarMemCopy(p->dst, p->src, p->len);
	}

	/* initialize local data */
	FarMemCopy(&slaveConfigFB_CPU0, &__const__slaveConfigFB_CPU0__param_init, sizeof __const__slaveConfigFB_CPU0__param_init);
...
void slaveConfigFB_CPU0__call(void)
{
	...
	slaveConfigFB_CPU0.ENO = slaveConfigFB_CPU0.EN;
	if ( ! slaveConfigFB_CPU0.EN )
	{
		return;
	}
...

	{
		...
		VT3_CONFIGURE_SLAVE__call(&vt3_configure_slave_IO_CORE_CPU0);

		if ( EQ_BOOL_2((vt3_configure_slave_IO_CORE_CPU0.is_busy),
			FALSE) ) {
			if ( EQ_BYTE_2((vt3_configure_slave_IO_CORE_CPU0.progress),
				((SINT)0)) ) {
				vt3_configure_slave_IO_CORE_CPU0.start = TRUE;
			} else if  ( NE_BYTE((vt3_configure_slave_IO_CORE_CPU0.progress),
				((SINT)100)) ) {
				
			} else if  ( TRUE ) {
				vt3_configure_slave_IO_CORE_CPU0.start = FALSE;
			}
		} else if  ( TRUE ) {
...
#endif /* INCLUDE__VT3_POU__SLAVECONFIGFB_CPU0_C */
